      ...
      <Tab v-for="event in events" :key="event.id" :title="event.name">
        <div class="w-full flex justify-center">
          <div class="grid grid-cols-3 container gap-16 w-full h-full">
            <div @click="openModal(competition)" v-for="competition in event.competitions" :key="competition.id" class="group relative shadow-lg shadow-gold-200 p-5 items-center h-40 rounded-[2rem] bg-gray-900 pb-2 flex overflow-hidden">
